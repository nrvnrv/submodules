...
async function createDbApp() {
  let dbApp = await DbAppODBC.create(loadEnv(dotEnvPath), {
...
async function createDbAppPromise() {
  let dbApp = DbAppODBC.createPromise(loadEnv(dotEnvPath), {
    dsn: "SYBASE_DSN_DOCKER",
    uid: "SYBASE_USERNAME",
    pwd: "SYBASE_PASSWORD",
  });
...
async function main() {
  ...
  for (let i = 0; i <= numToSend; i++) {
    ...
    dbApp.executeQueryTag`select count(*), ${i} from diagnose`.then(
      (result) => {
...
async function main2_1() {
  let dbApp = await createDbApp();
  dbApp
    .executeQuery(`SELECT TOP(2) * FROM EXPERTS2`)
    .then((result) => {
      console.log(result);
    })
    .catch((result) => {
