...
def form_artifacts_list(
    root_folder: str, submod_folder: str, search_dict: dict
) -> list:
    """
    Сформировать словарь для функции :func:`update_pathes.update_pathes`.
    Формат словаря приведен в описании функции :func:`update_pathes.update_pathes`
    ...
    for i in range(len(searched_dirs)):
        for j in range(len(searched_dirs)):
            if (
                (i != j)
                and (searched_dirs[i] != None)
                and (searched_dirs[j] != None)
                ...
                module_name = ".".join(
                    os.path.normpath(searched_dirs[i]["path"]).split("\\")[2:]
                )

                result_pathes = update_res_dirs(
                    result_pathes,
                    language,
                    {
                        module_name: {
                            searched_dirs[i]["obj"]: {
                                ...
                                "file": getFileList(
                                    os.path.join(
                                        root_folder,
                                        searched_dirs[i]["path"],
                                        search_dict[searched_dirs[i]["obj"]],
                                    )
                                ),
                            },
                            searched_dirs[j]["obj"]: {
                                ...
                                "file": getFileList(
                                    os.path.join(
                                        root_folder,
                                        searched_dirs[j]["path"],
                                        search_dict[searched_dirs[j]["obj"]],
                                    )
                                ),
                            },
                        }
